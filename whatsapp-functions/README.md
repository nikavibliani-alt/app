...
| `summarizeGuestConversation` | `onDocumentWritten` on `reservations/{docId}` | Once per checkout (after the checkout day ends in Tbilisi, reservation not cancelled), summarizes that stay onto `whatsapp_checkout_summaries/{reservationNumber}` and as the latest summary in `whatsapp_guests/{phone}.summary`. Never deletes messages. See "Post-checkout summary" below. |
...
- **Runs once per checkout.** It atomically claims
  ...
  failed summary releases the claim so a later write retries.
- **Never deletes messages.** The full raw conversation in
  `whatsapp_conversations/{phone}/messages` (guest, bot and owner) is kept
  forever. It only reads the stay's messages: those after the previous
  summarized stay's checkout day and before this one's end.
- **Keeps every stay's summary** on its marker doc (`summary`, `stay`), and
